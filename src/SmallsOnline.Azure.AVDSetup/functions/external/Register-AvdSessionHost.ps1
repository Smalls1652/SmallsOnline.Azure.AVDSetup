function Register-AvdSessionHost {
    ...
    $invokeAvdAgentInstallScriptBlock = {
    ...
    $tempFilePath = Join-Path -Path ([System.IO.Path]::GetTempPath()) -ChildPath "Invoke-AvdAgentInstall.ps1"
    $invokeAvdAgentInstallScriptBlock.ToString() | Out-File -FilePath $tempFilePath

    Invoke-AzVMRunCommand -VM $VM -CommandId "RunPowerShellScript" -ScriptPath $tempFilePath -Parameter @{ "RegistrationToken" = $RegistrationToken; } -AsJob
